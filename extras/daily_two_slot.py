...
def main(argv: List[str]) -> int:
    ...
    p.add_argument(
        "--state-file",
        default=os.path.join(os.path.expanduser("~"), "scoutsignal-config", ".daily-two-slot-state.json"),
        help="JSON state path (default: ~/scoutsignal-config/.daily-two-slot-state.json)",
    )
    p.add_argument(
        "--morning-start-hour",
        type=int,
